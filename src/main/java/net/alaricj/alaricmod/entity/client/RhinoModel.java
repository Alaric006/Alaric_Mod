package net.alaricj.alaricmod.entity.client;// Made with Blockbench 4.10.1
...
import net.alaricj.alaricmod.entity.animations.ModAnimationDefinitions;
...
public class RhinoModel<T extends Entity> extends HierarchicalModel<T> {
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.animate( ((RhinoEntity) entity).idleAnimationState, ModAnimationDefinitions.RHINO_IDLE, ageInTicks);
		this.animate( ((RhinoEntity) entity).attackAnimationState, ModAnimationDefinitions.RHINO_ATTACK, ageInTicks);
	}

	private void applyHeadRotation(float pNetHeadYaw, float pHeadPitch, float pAgeInTicks) {
		pNetHeadYaw = Mth.clamp(pNetHeadYaw,-30f, 30f);
		pHeadPitch = Mth.clamp(pHeadPitch, -25f, 40f);
